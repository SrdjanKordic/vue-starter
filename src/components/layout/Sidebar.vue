<template>
	<div class="app-sidebar bg-white rounded-2 p-sm-3">
		<div
			...
			@click="$store.commit('TOGGLE_MENU', false)"
		></div>
		<nav
		...
		>
			...
			<ul class="list-unstyled mb-0 py-3 pt-md-1">
				<li class="mb-2">
					<router-link class="btn text-start px-3" to="/">
						<font-awesome-icon :icon="['fas', 'house-user']" class="me-1" /> Dashboard
					</router-link>
				</li>
				<li class="mb-2">
				...
				<li class="mb-2">
					<router-link class="btn text-start px-3" to="/ui-kit">
						<font-awesome-icon :icon="['fas', 'box']" class="me-1" /> UI Kit
					</router-link>
				</li>
				<li class="mb-2" v-if="isAuth"><div class="text-warning">Settings</div></li>
				<li class="mb-2" v-if="isAuth && authUser.permissions.includes('USER_ACCESS')">
					<router-link class="btn text-start px-3" to="/users">
						<font-awesome-icon :icon="['fas', 'users']" class="me-1" /> Users
					</router-link>
				</li>
				<li class="mb-2" v-if="isAuth && authUser.permissions.includes('SETTINGS_GENERAL_ACCESS')">
